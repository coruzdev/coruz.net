...
export function createContactURL(type: string, message: string = DEFAULT_MESSAGE) {
  if (type === 'w') return `https://wa.me/${CONTACT_INFO.whatsAppNumber}?text=${encodeURI(message)}`
  if (type === 'e-info') {
  ...
  if (type === 'e-arco') {
    return `mailto:${CONTACT_INFO.contactEmail}?subject=${encodeURI('Derechos ARCO')}`
  }
  if (type === 'e-problem') {
